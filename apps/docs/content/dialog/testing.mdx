...
function DeleteButton() {
  ...
  return (
    <button onClick={async () => console.log(await confirm('Delete?'))}>Delete</button>
  );
}
...
Only reach for `DialogsProvider` + `useDialogs()` when you're testing the *opening* logic
(the component that calls `open()`/`confirm()`/etc.), not the dialog UI itself.

## Reference

`@omnireact/dialog`'s own test suite
([`packages/dialog/src/dialog.test.tsx`](https://github.com/jakhang/omnireact/blob/main/packages/dialog/src/dialog.test.tsx))

...
Try all four ways to open a dialog — the log below each result shows exactly what each
call resolved with:
...
- **`open(custom)`** — opens `RatingDialog`, a component with its own payload
  (`{ itemName: string }`) and result (`number | null`) that has nothing to do with the
  three built-in kinds above — this is the escape hatch for any dialog shape you need.
...
<Tabs items={['Preview', 'Code']}>
  ...
  <Tabs.Tab>
    ...
    ([view on GitHub](https://github.com/jakhang/omnireact/blob/main/apps/docs/components/tailwind-dialog-demo.tsx)):
...
call `open(templates.alert, ...)`/`open(templates.confirm, ...)`/`open(templates.prompt,
...)` internally. See the [API reference](/dialog/api-reference) for the full breakdown.

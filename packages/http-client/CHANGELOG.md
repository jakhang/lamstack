...
- 6a0c828: Initial release of `@lamstack/http-client` — a framework-agnostic HTTP client core with a
  ...
  - **`withHeaders`/`withMeta`** — pure helpers in `core/` for producing a copy of a resolved
    `HttpRequest` with headers or `meta` merged on top, using the same normalization rules as
    `resolve()` (header keys lowercased, `null`/`undefined` deletes a key). `bearer()`,
    `apiKey()`, `basic()`, and `recover()`'s internal meta bookkeeping use them instead of
    hand-rolled spreads, so a plugin option with a differently-cased header name (e.g.
    `{ header: 'X-Api-Key' }`) correctly overwrites rather than duplicating an existing
    header.

  - **`metaOptOut()` and a symmetric `skip?`/`order?` on `auth`/`recover`/`errorMapper`** —
    all three plugins read `meta` opt-out flags consistently, defaulting to
    `metaOptOut('auth' | 'recover' | 'mapError')` — a strict `meta[key] === false` check — via
    a shared `options.skip`, checked independently of and before `shouldRecover` runs.

  - **`cooldownMs` on `recover()`** (default `1000`ms, `0` disables) — stops a "refresh
    storm": every request that fails while the refresh endpoint itself is down would
    otherwise start its own brand-new recovery cycle against that same down endpoint, with no
    ceiling. Instead, after a cycle fails, any request that would start a fresh one within
    `cooldownMs` throws its own original error immediately (with the cached recovery failure
    attached via `.cause`) and emits `recovery:unavailable`, with no new attempt against the
    refresh endpoint. A successful cycle resets the cooldown immediately.
  ...
  - **`maxStaleRetries` on `recover()`** (default `1`), independent of `maxAttempts` — a
    ...
    credential ("stale generation") is tracked by its own counter and capped separately, so
    it can't exhaust the same attempt budget as a genuine recovery cycle; exceeding
    `maxStaleRetries` throws the original error with no new cycle.
